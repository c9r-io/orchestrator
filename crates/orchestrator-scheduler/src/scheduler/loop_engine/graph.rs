...
pub(super) async fn execute_cycle_graph(
    ...
    if let Some(action) = cycle_safety::check_invariants(
        ...
        task_id,
        task_ctx,
        agent_orchestrator::config::InvariantCheckPoint::BeforeCycle,
    )
    .await?
    {
        return match action {
...
async fn execute_graph_nodes(
    ...
    items: &mut [agent_orchestrator::dto::TaskItemRow],
...
fn evaluate_edge(
    ...
    agent_orchestrator::prehook::evaluate_step_prehook_expression(condition, &ctx)
}
...
async fn inject_dynamic_pool_steps(
    ...
    for item in items {
        ...
        let mut pool = agent_orchestrator::dynamic_orchestration::DynamicStepPool::new();
...
async fn insert_graph_run(
    ...
    state
        .task_repo
        .insert_task_graph_run(agent_orchestrator::task_repository::NewTaskGraphRun {
...
async fn persist_graph_snapshot_payload(
    ...
    state
        .task_repo
        .insert_task_graph_snapshot(agent_orchestrator::task_repository::NewTaskGraphSnapshot {
