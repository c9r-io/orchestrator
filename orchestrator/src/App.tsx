...
function defaultWorkflowSteps() {
  return [
    { id: 'init_once', type: 'init_once' as const, enabled: false, repeatable: true, is_guard: false },
    { id: 'qa', type: 'qa' as const, enabled: true, repeatable: true, is_guard: false },
    { id: 'ticket_scan', type: 'ticket_scan' as const, enabled: false, repeatable: true, is_guard: false },
    { id: 'fix', type: 'fix' as const, enabled: false, repeatable: true, is_guard: false },
    { id: 'retest', type: 'retest' as const, enabled: false, repeatable: true, is_guard: false }
  ];
}
...
function ensureWorkflowShape(config: OrchestratorConfigModel) {
  for (const workflow of Object.values(config.workflows)) {
    ...
    workflow.steps = WORKFLOW_STEP_ORDER.map((stepType) => {
      ...
      return { id: stepType, type: stepType, enabled: false, repeatable: true, is_guard: false };
    });
    workflow.loop = workflow.loop ?? {
      mode: 'once',
      guard: { enabled: true, stop_when_no_unresolved: true }
    };
...
export function App() {
  ...
  useEffect(() => {
    const saved = (localStorage.getItem('orchestrator-theme') as Theme | null) ?? 'dark';
