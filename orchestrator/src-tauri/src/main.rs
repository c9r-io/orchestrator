...
async fn stop_task_runtime_for_delete(state: Arc<InnerState>, task_id: &str) -> Result<()> {
    ...
    if let Some(runtime) = runtime {
        println!(
            "[agent-orchestrator][delete] runtime found, sending stop/kill task_id={}",
            task_id
        );
        runtime.stop_flag.store(true, Ordering::SeqCst);
        kill_current_child(&runtime).await;
        println!(
            "[agent-orchestrator][delete] stop/kill dispatched task_id={}",
            task_id
        );
    } else {
        println!(
            "[agent-orchestrator][delete] no runtime found task_id={}, continue delete",
            task_id
        );
    }
    Ok(())
...
fn resolve_cli_task_id(state: &InnerState, options: &CliOptions) -> Result<String> {
    ...
    if !options.no_auto_resume {
        if let Some(task_id) = find_latest_resumable_task_id(state, true)? {
            println!(
                "[agent-orchestrator][cli] auto-selected existing task: {}",
                task_id
            );
            return Ok(task_id);
        }
    ...
    println!("[agent-orchestrator][cli] created task: {}", created.id);
    Ok(created.id)
}
...
async fn run_cli_mode_async(state: Arc<InnerState>, options: CliOptions) -> Result<i32> {
    ...
    println!(
        "[agent-orchestrator][cli] finished task={} status={} finished={}/{} failed={}",
        summary.id,
        summary.status,
...
fn print_startup_banner(state: &InnerState) {
    println!("[agent-orchestrator] app_root={}", state.app_root.display());
    if let Ok(active) = read_active_config(state) {
        println!(
            "[agent-orchestrator] default_workspace={}",
            active.default_workspace_id
        );
        println!(
            "[agent-orchestrator] default_workflow={}",
            active.default_workflow_id
        );
    }
    println!("[agent-orchestrator] db_path={}", state.db_path.display());
}
