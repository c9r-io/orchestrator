...
cleanup() {
  ...
  if [[ "$FAIL" -gt 0 || "${KEEP_FR116_QA:-0}" == "1" ]] &&
...
trap cleanup EXIT
gate_runlog_arm "scripts/qa/test-agent-driver-abstraction.sh"
...
cd "$REPO_ROOT"
if [[ "${FR116_ALLOW_DIRTY:-0}" != "1" && -n "$(gate_runlog_worktree_status "$REPO_ROOT")" ]]; then
