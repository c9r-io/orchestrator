...
cleanup() {
  ...
  if [[ "$FAILED" == "1" || "${KEEP_FR115_QA:-0}" == "1" ]] && gate_scratch_has_evidence "$LOG_ROOT"; then
...
trap cleanup EXIT
gate_runlog_arm "scripts/qa/test-slack-dedicated-app-provisioning.sh"
...
cd "$REPO_ROOT"
if [[ "${FR115_ALLOW_DIRTY:-0}" != "1" && -n "$(gate_runlog_worktree_status "$REPO_ROOT")" ]]; then
