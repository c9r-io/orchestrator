...
echo "[qa-doc-lint] Checking error-code glossary parity..."
if ! ./scripts/qa/test-error-code-glossary.sh --fixture-test; then
  fail=1
fi
...
while IFS=: read -r file line _; do
  ...
  rg -n "task create" docs/qa -g '*.md' \
    | rg -v -e "task create --help" -e "task create --format" -e "does not depend on" -e "task create supports" -e "task create does NOT" -e "task create --no-start is defined" -e "^\S+:\d+:\|" -e "\`task create\` supports" -e "\`task create\` does NOT" -e "\`task create --no-start\` creates" -e "\`task create --no-start\` is defined" -e "^\S+:\d+:#" || true
)
...
# The "site guide files must not be tracked" assertion that used to live here now
# belongs to scripts/qa/test-docs-publishing-integrity.sh, which reads the published
# collections from config/governance/docs-publishing.json instead of naming two guide
# paths. Both run in the ci.yml governance job. Kept in one place so that adding a
# collection does not silently leave this copy behind checking the old two.
