...
11. **A measure that separates today's instances is not the same as a measure of the property.** FR-166 needed to know which files in the English guide directory are written in Chinese. The obvious measure — Han characters over Latin characters — is a measure of *glyph density*, and technical Chinese is dense with Latin tokens: CLI flags, YAML keys, product names. The four Chinese runbooks scored 0.44–0.52 and a partly bilingual English chapter scored 0.15, so every threshold either takes the English chapter or misses a runbook, and the first attempt caught **one of four** while looking like it worked. Asking the question actually being asked — what fraction of prose *lines* are written in Chinese — gives 0.94–0.97, 0.37 and 0.00, and the threshold lands in a wide empty band. Two habits: when a proxy needs a threshold, **print the distribution before choosing one** and check whether the band is empty or crowded; and if the population separates only under a carefully tuned constant, the measure is probably of the wrong quantity, not the right quantity needing tuning. A margin is evidence and belongs in the design record, because the next person to widen the scope needs to know how much room there was.

12. **A gate that fails twice keeps its first diagnosis.** FR-166's certification ran `coverage-governance.sh` twice. The first run failed because the GUI suite ran under a Node outside the declared range, which was true and environmental and was recorded as such. The second run, under a supported Node, failed **again** — this time because the FR's own Playwright assertions were stale — and the standing "environmental" label very nearly absorbed it. Nothing about a repeat failure announces that the cause changed; the exit code is the same, the gate name is the same, and the earlier conclusion is sitting right there. **Re-read the log on every failure, including the ones you have already explained**, and prefer classifications that name a mechanism (`cargo deny is not installed`) over ones that name a category (`environmental`), because the first is falsified by the next run and the second is not.

13. **A name is not an identity, and a repair aimed at one bug can build a worse one.** FR-169 taught a daemon whose data directory vanished to exit; its design record called the residue a bounded ~15s overlap. FR-170's step 0 measured the *end* of that window and found the opposite: teardown named its socket and pidfile by **path**, and after delete-and-recreate those paths belong to the successor, so the dying daemon unlinked the live daemon's files and left it alive, unreachable, unstoppable and permanently outside the watcher that would have ended it — with the pidfile gone, a third process then started cleanly. The FR-169 fix manufactured exactly the orphan it was written to prevent, and the same codebase already contained the correct reasoning, applied to the data directory and never to the two files beside it.

    Three habits. **When code identifies a resource by a name that something else can take — a path, a port, a PID, a URL, a key — ask what happens when it has been taken**, and prefer evidence that travels with the object (an inode, a recorded content, a generation counter) over evidence that travels with the name. **Whenever a fix adds an action on a failure path — an exit, a cleanup, a rollback, a retry — ask what that action does to whatever occupied the world in the meantime**; the previous FR's teardown was correct for the only world its author pictured. And most transferably: **when a design record states a residue as bounded, measure the boundary rather than inheriting it.** "Bounded, not closed" reads like a completed assessment and is a prediction; here the true residue was unbounded, and the sentence had been written three commits earlier by the same effort that measured everything else correctly.

    The same FR carried the shape a second time, in its own gate — twice, and neither was visible until the gate was run against the broken state it exists to catch. First, the `inode_of` helper returned non-zero for a missing socket, `set -e` took the enclosing `INO="$(inode_of …)"` assignment, and the gate printed two PASS lines, **no FAIL line and no summary** — a truncated run, indistinguishable from a complete one to anyone reading the exit code (§4.6 condition 5, arriving through a helper nobody would think to attack). Second, and worse: the probe for "a third daemon is refused" ran `orchestratord --foreground` inside a command substitution, which **assumes the refusal it is testing for**. Under the regression the daemon starts and never returns, so the gate **hung**. A hung gate is worse than a red one — CI reports a timeout with no diagnostic, no summary and no named subject — and on macOS there is no `timeout(1)` to fall back on, so the bound has to be written.

    Three rules follow. **A helper called inside a `set -e` assignment must be total.** **A gate should announce its own truncation** — an EXIT trap that fires when the summary line was never reached costs three lines and converts a silent abort into a stated one. And **any probe whose expected outcome is "this fails fast" must bound the case where it succeeds**, reclaiming whatever it started; otherwise the negative fixture's failure mode is a hang. Run every new gate against the broken state it exists to catch, and check that it produced a *verdict* — not merely a non-zero code, and not a timeout.
