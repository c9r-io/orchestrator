//! Synchronous task-execution repository traits.
//!
//! The original monolithic `TaskRepository` is now composed from seven
//! domain-aligned sub-traits.  A blanket implementation keeps the
//! `TaskRepository` supertrait backward-compatible — any type that
//! implements all sub-traits automatically implements `TaskRepository`.
//!
//! ## Sub-traits
//!
//! | Trait | Domain |
//! |-------|--------|
//! | [`TaskQueryRepository`] | Task discovery, lookup, and metadata |
//! | [`TaskItemQueryRepository`] | Item enumeration and health-check queries |
//! | [`TaskStateRepository`] | Task lifecycle state mutations |
//! | [`TaskItemMutRepository`] | Task item state mutations |
//! | [`CommandRunRepository`] | Command run recording and process tracking |
...
// ── Composite supertrait (backward-compatible) ──────────────────────

/// Full task repository — backward-compatible supertrait composing all
/// domain-specific sub-traits.
///
...
impl<T> TaskRepository for T where
    T: TaskQueryRepository
        + TaskItemQueryRepository
        + TaskStateRepository
        + TaskItemMutRepository
        + CommandRunRepository
