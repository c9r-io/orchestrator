...
impl AsyncSqliteTaskRepository {
    ...
    pub async fn resolve_task_id(&self, prefix: &str) -> Result<String> {
        ...
        self.async_db.reader().call(move |conn| {
    ...
    pub async fn load_task_detail_rows(
        &self,
        task_id: &str,
    ) -> Result<(Vec<crate::dto::TaskItemDto>, Vec<crate::dto::CommandRunDto>, Vec<crate::dto::EventDto>)> {
        let task_id = task_id.to_owned();
        self.async_db.reader().call(move |conn| {
            queries::load_task_detail_rows(conn, &task_id)
                .map_err(|e| tokio_rusqlite::Error::Other(e.into()))
    ...
    pub async fn list_task_items_for_cycle(&self, task_id: &str) -> Result<Vec<crate::dto::TaskItemRow>> {
        let task_id = task_id.to_owned();
        self.async_db.reader().call(move |conn| {
            queries::list_task_items_for_cycle(conn, &task_id)
                .map_err(|e| tokio_rusqlite::Error::Other(e.into()))
    ...
    pub async fn set_task_status(&self, task_id: &str, status: &str, set_completed: bool) -> Result<()> {
        ...
        self.async_db.writer().call(move |conn| {
            state::set_task_status(conn, &task_id, &status, set_completed)
                .map_err(|e| tokio_rusqlite::Error::Other(e.into()))
        }).await.map_err(flatten_err)
    }
