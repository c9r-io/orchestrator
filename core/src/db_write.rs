...
//! is re-exported here so every existing `crate::db_write::*` and
//! `agent_orchestrator::db_write::*` path keeps resolving.
//!
//! The tests stay in core because they build their fixtures with
//! `crate::test_utils::TestState` and `crate::task_ops::create_task_impl` —
//! domain machinery that sits above this layer and cannot sink below it.
