...
Two further gates that were deterministic and daemon-free but unwired — `test-codex-session-resume.sh` (asserts a recorded transcript; the live counterpart is the separate `certify-codex-session-resume.sh`) and `test-legacy-coordination-decommission.sh` — were also promoted to `ci-required`. The surface was 12 `ci-required` of 45 gates at FR-127's closure. Later FRs land on the surface they were given: FR-128 added `test-governance-ledger-tooling.sh`, FR-129 the skill mirror gates, and FR-130 `core-boundary.rb` with `test-core-boundary.sh`, making it 16 of 49. The number is not the invariant — that every gate on disk is classified is.
