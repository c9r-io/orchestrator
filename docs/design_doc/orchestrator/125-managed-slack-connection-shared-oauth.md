...
A controlled Slack sandbox certification remains a separate, non-CI gate because it requires real workspace consent and external credentials. The 2026-07-21/22 run passed L0-L11 with two workspaces, two isolated daemons, real OAuth, real reactions, transfer, revocation, backup/restore, disconnect, uninstall, and privacy cleanup. Future runs use the FR-123 `scripts/qa/certify-slack-managed-live.sh run --mode shared|both` checkpoint controller with an operator-owned mode-0600 environment file and a separate sandbox test-driver App. Evidence freshness, safe promotion, and cleanup inventory are governed by `docs/design_doc/orchestrator/135-slack-sandbox-continuous-certification.md` and QA-173; dedicated provider behavior remains specified by QA-163.
