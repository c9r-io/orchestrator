...
- Legacy clients silently bypass required semantics.
  ...
  - The `Delete` RPC did not conform until FR-167, and it failed a second way the sentence above does not cover. Its guard was `force_references || is_source_task_binding`, with **no** `context.is_some()` disjunct — so unlike apply it discarded an envelope a client had correctly sent, and the CLI sends one on every delete. Eleven of the twelve kinds left no row at all, and `enforced` neither recorded a delete nor refused it, on the one mutation that cannot be undone. Every non-dry-run delete now reserves an envelope, named per kind. The narrower lesson: "the audit layer is bypassed for callers who send nothing" was the *apply* instance, not the shape. State the shape as a claim about the guard, not about the caller.
  - Counted across `crates/daemon/src/server/` at `d7ef4faf`, after FR-167: every production `action_audit::begin` call site is now unconditional. `Delete` was the last conditional one.
