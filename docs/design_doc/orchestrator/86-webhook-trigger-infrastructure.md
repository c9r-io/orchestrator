...
The daemon spawns a lightweight axum HTTP server on a separate port (default `127.0.0.1:19090`, loopback only). Use `--webhook-bind` to override or `none` to disable. Non-loopback addresses require a webhook secret or `--webhook-allow-unsigned`. Shares `InnerState` with the gRPC server.
