...
16 个门禁被留成 `gate_daemon_wait_ready "$ORCH" || true` 后面跟着它们原来的
`if ! "$ORCH" task list -o json` 守卫——helper 的判定被丢掉，守卫用**它刚替换掉的
那个代理**复探。于是"套接字起来了、worker 还没注册"的 daemon 照样通过守卫，
门禁照跑不误。**行为一点没变**。

用 shim 实测（除就绪调用外全部转发，就绪调用先 sleep 8 秒再拒绝，
使套接字确实答得出来）：修复前门禁**退出 0 并报 `10 passed, 0 failed`**，
修复后退出 1、0 个场景执行。那句"10 passed"就是本需求存在的全部理由。

守卫现在直接测就绪并保留各自的诊断（daemon.log 转储），两处差异具名：
`test-fr013-control-plane-protection.sh` 在此之前**根本没有就绪守卫**，
旧循环静默落空、正文对着任意状态开跑，现在有东西会报了，失败直接经 `set -e` 传播；
`test-failure-visibility.sh` 的密钥文件等待保留为第二个等待，就绪守卫移到它前面。
