...
| 文件 | 改动 |
|------|------|
| `crates/orchestrator-scheduler/src/scheduler/loop_engine/mod.rs` | 新增 `proactive_max_cycles()` 函数；在 cycle 递增前调用检查并 emit `max_cycles_enforced` 事件 |
| `crates/orchestrator-scheduler/src/scheduler/loop_engine/tests.rs` | 5 个单元测试覆盖 Fixed/Infinite/Once 模式 |
