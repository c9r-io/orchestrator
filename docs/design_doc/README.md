...
Every design doc opens with YAML frontmatter declaring whether it still describes the live
mechanism. `scripts/qa/doc-lifecycle.rb` enforces this in CI and a new document without it fails
the build.
...
When a later change *replaces* the mechanism a document describes, set `lifecycle: superseded` and
add `superseded_by:` naming the successor's repository-relative path. Keep the document and its
prose banner: the gate reads the metadata, a reader reads the banner, and neither substitutes for
the other. A post-release update is not supersession — the document is still the live path.

This is a different axis from the `**Status**:` header many of these documents carry. That one
records implementation maturity at authoring time; a `Released` document can be superseded.

`config/governance/doc-lifecycle-index.json` is generated from this frontmatter and gives the
reverse directions the index table below cannot: document → feature request, and superseded →
successor. Regenerate it in the same commit as the document change:
...
| orchestrator | `docs/design_doc/orchestrator/101-streaming-agent-runner-architecture-pivot.md` | `docs/qa/orchestrator/164-agent-driver-abstraction.md`, `docs/qa/orchestrator/168-coordination-collapse-mcp-tools.md` | Released pivot record: provider-neutral structured drivers plus authenticated daemon-owned coordination tools collapse coordination out of YAML/CEL while retaining compatibility |
...
| orchestrator | `docs/design_doc/orchestrator/126-dedicated-slack-app-auto-provisioning.md` | `docs/qa/orchestrator/163-dedicated-slack-app-auto-provisioning.md` | Per-workspace private Slack App provisioning, local token custody, exact-App OAuth/events, reviewed lifecycle/migration, same-message badge fan-out, offline recovery, and controlled live certification |
| orchestrator | `docs/design_doc/orchestrator/127-agent-driver-abstraction.md` | `docs/qa/orchestrator/164-agent-driver-abstraction.md` | Provider-neutral shell/Claude/Codex drivers, apply-time capability gates, normalized event folding, session privacy, common sandbox/cancel paths, and private MCP configuration |
| orchestrator | `docs/design_doc/orchestrator/128-non-code-workspace-and-global-file-sharing.md` | `docs/qa/orchestrator/165-non-code-workspace-and-global-file-sharing.md`, `docs/qa/orchestrator/167-global-skill-directory-provenance.md` | Task workspaces, implicit process items, operator-owned file-sharing ceiling, isolated HOME, trusted global Skill provenance, Console semantics, and Slack pilot |
| orchestrator | `docs/design_doc/orchestrator/129-codex-session-resume-conformance.md` | `docs/qa/orchestrator/166-codex-session-resume-conformance.md` | Exact-version Codex resume grammar, real context continuity, sanitized protocol fixture, offline replay, and controlled live recertification |
| orchestrator | `docs/design_doc/orchestrator/130-coordination-collapse-mcp-tools.md` | `docs/qa/orchestrator/168-coordination-collapse-mcp-tools.md` | Authenticated per-run daemon tool host, transport-only stdio shim, six primary coordination tools, complete events, parity pilot, and measured residual channels |
| orchestrator | `docs/design_doc/orchestrator/131-expert-resources-governed-editing.md` | `docs/qa/orchestrator/169-expert-resources-governed-editing.md` | Five daemon-owned resource catalogs, canonical manifests, reviewed optimistic Apply, Action Audit privacy, and accessible Expert UI |
| orchestrator | `docs/design_doc/orchestrator/132-handoff-dialog-focus-lifecycle.md` | `docs/qa/orchestrator/170-handoff-dialog-focus-lifecycle.md` | Manual and Attention-initiated safe-resume review with validated focus sources, modal containment, deterministic fallbacks, and async lifecycle fencing |
| orchestrator | `docs/design_doc/orchestrator/133-attention-mutation-error-reconciliation.md` | `docs/qa/orchestrator/171-attention-mutation-error-reconciliation.md` | Independent query/stream/mutation errors, one-shot authoritative reconciliation, safe copy, focus recovery, and privacy-safe outcome metrics |
| orchestrator | `docs/design_doc/orchestrator/134-boundary-layer-coverage-governance.md` | `docs/qa/orchestrator/172-boundary-layer-coverage-governance.md` | Machine-readable component/module coverage, approved non-regression baseline, explicit branch support, daemon risk matrices, real CLI/Tauri tonic adapters, and FR evidence traceability |
| orchestrator | `docs/design_doc/orchestrator/135-slack-sandbox-continuous-certification.md` | `docs/qa/orchestrator/173-slack-sandbox-continuous-certification.md` | Unified shared/dedicated live modes, resumable provider checkpoints, inert minimal secrets, expiring safe evidence, recorded CI contracts, and reviewed cleanup |
| orchestrator | `docs/design_doc/orchestrator/136-coordination-strangler-completion.md` | `docs/qa/orchestrator/174-coordination-strangler-completion.md` | Exact production workflow inventory, seven typed-tool migrations, explicit session/tool boundaries, exact-equality legacy ratchet, retirement criteria, and isolated two-cycle parity |
| orchestrator | `docs/design_doc/orchestrator/137-legacy-coordination-decommission.md` | `docs/qa/orchestrator/175-legacy-coordination-decommission.md` | Capture/JSONPath production removal, narrow residual state, exact generic-variable and command-only Agent blockers, post-retirement tool regression, and exact-equality retirement evidence |
| orchestrator | `docs/design_doc/orchestrator/138-agent-driver-execution-migration.md` | `docs/qa/orchestrator/176-agent-driver-execution-migration.md` | Per-Agent typed-driver inventory, production-contract offline parity, typed tool convergence, admission/compatibility separation, EN/ZH guide semantics ratchet, executable rollback, and mandatory release closure |
| orchestrator | `docs/design_doc/orchestrator/139-qa-gate-enforcement-surface.md` | `docs/qa/orchestrator/177-qa-gate-enforcement-surface.md` | Gate enforcement classification, bidirectional manifest/disk comparison, workflow wiring truth, provider-isolation invariant, and seven mutually isolated negative fixtures |
| orchestrator | `docs/design_doc/orchestrator/140-governance-ledger-regeneration.md` | `docs/qa/orchestrator/178-governance-ledger-regeneration.md` | Reviewed ledger regeneration (`--emit-inventory` / `--emit-baseline` / CI-refusing `--write`), candidate generated by the same expression the gate compares, and per-Agent mismatch reporting recovered from `git show HEAD:` |
| orchestrator | `docs/design_doc/orchestrator/141-skill-mirror-integrity.md` | `docs/qa/orchestrator/179-skill-mirror-integrity.md` | Single authoritative skill source, per-root bidirectional mirror coverage, canonical symlink shape, and a read check no structural test substitutes for |
| orchestrator | `docs/design_doc/orchestrator/142-core-boundary-freeze.md` | `docs/qa/orchestrator/180-core-boundary-freeze.md` | Core public surface and per-file `rusqlite` inventory frozen by exact equality, plus the reviewed migration schema baseline with idempotency and resume at all 37 interruption points |
| orchestrator | `docs/design_doc/orchestrator/143-docs-publishing-integrity.md` | `docs/qa/orchestrator/181-docs-publishing-integrity.md` | Declared publish set proven by running the generator and diffing trees, bidirectional navigation reachability, sync idempotency, and repo-wide relative-link resolution |
| orchestrator | `docs/design_doc/orchestrator/144-doc-lifecycle-governance.md` | `docs/qa/orchestrator/182-doc-lifecycle-governance.md` | DD/QA lifecycle frontmatter, supersession referential integrity including cycles, filesystem-derived coverage, and an exact-equality reverse index |
| orchestrator | `docs/design_doc/orchestrator/145-gate-surface-execution-truth.md` | `docs/qa/orchestrator/183-gate-surface-execution-truth.md` | Wiring, pinning and provider isolation decided by execution rather than by text; coverage discovered rather than enumerated; CI job liveness and environment parity |
| orchestrator | `docs/design_doc/orchestrator/146-bash32-compatibility.md` | `docs/qa/orchestrator/184-bash32-compatibility.md` | bash 3.2 hazard elimination with rules executed under a real 3.2, the coverage shell main path exercised behind stubs, and upload-step diagnostic fidelity |
| orchestrator | `docs/design_doc/orchestrator/147-persistence-dependency-chokepoint.md` | `docs/qa/orchestrator/185-persistence-dependency-chokepoint.md` | Who may hold the SQLite driver, decided before the extraction and scoped to the orchestrator database; a two-condition gate because the manifest alone is a proxy |
| orchestrator | `docs/design_doc/orchestrator/148-persistence-crate-extraction.md` | `docs/qa/orchestrator/186-persistence-crate-extraction.md` | FR-130 Phase A and C: the persistence layer extracted below core in four revertible commits, why `dto` moved whole and `config.rs` did not, the driver-error conversion answered by measurement rather than by choosing between two proposed designs, and the reference count shown to be 61% error plumbing with the resulting shortcut refused |
| orchestrator | `docs/design_doc/orchestrator/149-governance-aggregation-completeness.md` | `docs/qa/orchestrator/187-governance-aggregation-completeness.md` | FR-137: the `governance` job's outcome list derived from the parsed workflow instead of hand-written, the three ways a swallowed step failure disappears each with its own fixture, and the aggregate script executed to prove the outcomes decide the job |
| orchestrator | `docs/design_doc/orchestrator/150-trigger-history-limit-cascade.md` | `docs/qa/orchestrator/188-trigger-history-limit-cascade.md` | FR-142: why a documented `historyLimit` had never deleted a task and had no symptom, the A/B/C decision reworked after none proved implementable, the blocking-reference set read from the schema rather than listed, skipped-whole-and-named instead of stripped, and the same defect recorded as still live in `task_cleanup` and `task delete` |
| orchestrator | `docs/design_doc/orchestrator/152-shell-lexical-state.md` | `docs/qa/orchestrator/190-bash32-scanner-lexical-state.md` | FR-138: why the bash 3.2 scanner stopped reading at the first here-document lookalike, the `$( )` quoting reset that made the FR's own stated fix insufficient for the site it was filed over, the unclosed-here-document backstop that holds whatever the lexer gets wrong, per-file line accounting counted rather than derived, and per-file emptiness inference replaced by matching |
| orchestrator | `docs/design_doc/orchestrator/153-governance-execution-cost.md` | `docs/qa/orchestrator/191-governance-execution-cost.md` | FR-140: what each governance gate costs, discovered by reading `ci.yml` and the enforcement surface together rather than from a list; why a duration ledger compares against a ceiling where `sourceBaseline` compares for equality; the 45-minute budget, its non-circular rationale and its review condition; and the measurement that redirected the FR — per-case tree copying is 3.2% of the gate it was blamed for, while `RustLexer.mask_literals` walking 6.4 MB one `String#[]` at a time was 37 of the seconds |
| orchestrator | `docs/design_doc/orchestrator/154-jq-status-observed.md` | `docs/qa/orchestrator/192-jq-status-observed.md` | FR-144: why a gate could report PASS over a manifest jq could not parse — a process substitution's exit status is nobody's, and condition position disables `set -e` for the whole call tree beneath it; why an empty result must be declared `require-rows` or `allow-empty` with no default, since it fails closed in one check and open in another; why capture-and-test was not sufficient for reads nested four loops deep and a failure record a subshell can write was needed; and the scanner that parses rather than greps, because the documents forbidding the pattern must quote it |
| orchestrator | `docs/design_doc/orchestrator/155-fixture-target-drift.md` | `docs/qa/orchestrator/193-fixture-target-drift.md` | FR-143: the nine recorded times a negative fixture's target moved out from under it and eight stayed green, including the one that reported a defect the gate did not have; why the third meta assertion had to be a gate over the fixture scripts rather than an entry beside FR-129's two, since none of the three gates the FR cited as evidence has a check registry; why the `abort` was never the defect and being uncaught was; the parse that decides where to look paired with the raw text that decides what is there; and why the lighter form suffices, with the residue it does not close named — a gate already failing before the mutation |
| orchestrator | `docs/design_doc/orchestrator/guide-alignment.md` | `docs/qa/orchestrator/guide-alignment.md` | Compile-driven EN/ZH CLI guide review plus deterministic high-risk Agent driver semantics ratchet |
