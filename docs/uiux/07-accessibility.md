...
- FR-113 release overlay: enter through visible "Sources" → "Automations", follow one real route to Source/Attention/Process Workspace, and combine the production Tauri provenance bridge with Playwright axe, role, redaction, focus, reduced-effect, and 640 px checks. See `docs/qa/orchestrator/161-slack-reaction-skill-automation-release.md` Scenario 4.
- FR-114/FR-115 overlay: the three provisioning choices remain readable and distinguish recommended, advanced, and unavailable states without color alone. The Configuration Token is a labelled password control with autocomplete disabled and clears on submit. Manifest permission expansion is textual; approval/OAuth/recovery/disconnect dialogs have visible status/error announcements, keyboard focus containment/return, and no hidden focusable Admin mutation for lower roles. Test desktop/640 px, axe, reduced effects, popup-blocked and resumed intent/provisioning states with QA-162 Scenario 4 and QA-163 Scenario 5.
