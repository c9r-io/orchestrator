...
- FR-114 overlay: the three provisioning choices remain readable and distinguish unavailable Dedicated mode without color alone. OAuth/cancel/reauthorize/disconnect dialogs have labelled controls, visible status/error announcements, keyboard focus containment/return, and no hidden focusable Admin mutation for lower roles. Test desktop/640 px, axe, reduced effects, popup-blocked and resumed-intent states with `docs/qa/orchestrator/162-managed-slack-connection-shared-oauth.md` Scenario 4.
