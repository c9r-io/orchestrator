...
1. 迁移 step 具体实现仍保留在 `core/src/migration.rs`，尚未拆分为独立的 `steps/*` 文件。
2. `db_write.rs`、scheduler、部分 config persistence 仍保留手写 SQL 与直接连接访问。
3. 尚未建立历史数据库样本库升级验证机制。
4. 尚未决定是否需要引入 `sqlx`、`SeaORM` 等替代迁移栈；当前代码仍基于 `rusqlite`。
...
- 当前已完成：
  ...
  - `core/src/migration.rs` 已退为兼容 facade + legacy step 实现承载点
