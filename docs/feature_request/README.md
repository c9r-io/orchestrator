...
> 由 `scripts/lib/fr_registry.rb` 从完整 `HEAD` 祖先历史生成：158 个历史编号 / 163 条历史路径，另有 13 条无 FR 文件历史的审阅例外；5 个编号存在多路径碰撞。浅克隆拒绝生成。
...
| FR-175 | 脱敏边界画在持久化时，而不是出网时 —— 两条已授权的读命令输出明文密钥 | P2 | Closed | git history |
