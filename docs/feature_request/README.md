...
| FR-139 | 持久化收口门禁的扫描面与断言有效性 | P2 | Proposed |
...
- FR-139 源自 FR-136 的闭环后审计，排在 FR-130 之前**不是因为阻塞 Phase A**（Phase A 的判据是 core 的 `rusqlite` token 收敛，由 `core-boundary.rb` 产出，与本文三条均不相干），而是因为冻结时机：Phase A 会把约 115 处引用、23 个文件从 core（该门禁排除）搬进新 crate（该门禁扫描），持久化台账要重生成并由人评审 `role` 与 23 条 `category`；若此时动词集仍缺 `PRAGMA`，那批数字从诞生起就错，修好后要再评审一次。三条：`classification_errors` 的总和分支比较的是同一个 hash 上的同一个归约，任何输入都无法使其失败，而 DD-147 的 Known Limits 把它当作现行保证陈述（覆盖面实际由 `reference_errors` 守住，故是记录过度声明加一段死分支）；`SQL_STATEMENT` 缺 `PRAGMA`，仅补该动词后总数 112 → 114，命中 `orchestrator-security/src/lib.rs:104` 与 `slack-gateway/src/store.rs:177`，零误报，其中前者正是 `exempt` crate 借连接执行 SQL 的形态而台账记为 `sql: 1`；`member_references` 只读 `<member>/src`，5 个 member 的 `build.rs`（含两个 `forbidden` crate）从不被读，而第 130 行把 `[build-dependencies]` 归入生产声明，且第 316 行的 scope 检查是散文比散文，无法发现常量本身描述的不是扫描所做的事
