...
| FR-140 | 治理执行成本 — 使其可见、可归因、可预算 | P3 | Proposed |
| FR-141 | 持久化层不再交出驱动连接 — `AsyncDatabase` 的 API 边界 | P1 | Proposed |
| FR-142 | 触发历史上限从未生效 — 级联删除的决策与修复 | P1 | Proposed |
...
- FR-142 是 FR-127 以来**唯一一条产品真的坏了**的立项，其余全部是治理或重构。`Trigger` 的 `history_limit` 是对外声明的配置项而从未生效过：`trigger_state.rs:212` 是裸 `DELETE FROM tasks`，而冻结 schema 中 `task_items` 引用 `tasks(id)` **无 `ON DELETE CASCADE`**（全库仅 2 处 CASCADE，都不在这条链上；共 10 张表引用 `tasks(id)`），任何有子行的 task 都删不掉。更重的一点是它**连症状都没有**——`trigger_engine.rs:652` 把失败记为 `debug!`，而 `daemon/src/main.rs:209` 的默认级别是 `info`，所以默认部署下一行日志都不会出现；同一函数上方两行的入队失败用的是 `error!`。正确的事务级联**已经存在**于 `task_repository/items.rs:88`（events → command_runs → task_items → tasks），`task_cleanup.rs` 走的就是它——所以这不是缺实现，是两条删除路径中触发历史走了错的那条。本 FR 是决策型（形态同 FR-136）：核心问题不是补一个 `DELETE`，而是回答历史上限有权删掉多少（命令运行记录连同磁盘日志、resume 计划与 handoff 快照、投递审计三类的答案可以不同），A/B/C 三种形态须书面选定。**这条缺陷是搬迁时被人眼发现的，62 道门禁没有一道能看见它**——它是一条被静默吞掉的运行时错误，记作 FR-140 讨论治理成本时的对照点
- FR-141 是 FR-130 Phase B 一次**正确的拒绝**留下的无主项。Phase B 拒绝了"给 `AsyncDatabase` 加闭包方法、删六份重复 `fn other`"的改动，因为它能不搬动任何 SQL 就把 83 收敛掉 39——推棘轮而非处置文件，拒绝成立。但那笔交易打包了两件事：推数字（该拒绝）与**持久化层不该把驱动的连接类型放在公开 API 上**（架构边界，从未被单独判断）。按后者，规模是 **165 个调用点**（core 126、daemon 27、scheduler 12，层内另有 67），不是 39 处引用。DD-147 早已点名它是门禁**必须有条件 2** 的全部理由——`secret_store_crypto.rs` 0 处驱动引用却跑 4 条生产 SQL 就是这个 API 的产物。更尖锐的是：两个 `forbidden` crate 有 39 处 `.writer()`/`.reader()` 调用而**只有 4 行同时带 `rusqlite` 词元**，其余约 35 处两个台账都数不到——条件 2 抓得住它们**用连接做了什么**，没有任何东西冻结**它们能拿到连接**。本 FR 排在 Phase B 之后（触及 core 126 个调用点，冲突面过大），且其副作用式的计数下降**不得**被读作 Phase B 进度
- FR-140 是 FR-127 以来第一条针对治理**成本**而非治理**正确性**的立项。2026-07-26 run `30189806915` 实测：`governance` 45 分钟、`ci-environment-parity` 22 分钟，而 `test` 4 分钟、`clippy` 1 分钟——治理占 67 分钟，产品自身测试占 5 分钟；最近 8 次 CI 端到端稳定在 40 分钟上下。当前 62 道门禁、29 道 `ci-required`，每个 FR 都在加，而 `ci-job-liveness.json` 只记结论不记时长、`qa-gate-surface.json` 只记分类不记代价，于是"再加一道门禁"永远是零成本决策。主要开销是隔离机制的**实现**——`new_case` 逐 case 全树复制（`test-qa-gate-surface.sh` 27 处、`test-persistence-dependency.sh` 24 处、`test-doc-lifecycle.sh` 与 `test-core-boundary.sh` 各 15 处），而多数 case 只读几个文件。隔离约定本身是对的且不得放弃，本 FR 只换实现。时长台账将是本仓库第一个正当使用阈值口径而非精确相等的台账，理由是被测量的量本身随机，须书面说明其与 FR-128 收紧 `sourceBaseline` 的取向差异
