...
| FR-126 | Agent 执行路径迁移 — Showcase 链接闭环与文档门禁补强 | P1 | In Progress |
...
- FR-126 在第三轮审计后曾闭环；后续审计发现 EN/ZH 指南链接的 mark-done showcase 仍保留已删除 executor 的当前操作说明，且确定性门禁未覆盖 `docs/showcases/`，因此已再次打开并补强链接闭环与目录级门禁。
