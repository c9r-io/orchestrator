...
> 由 `scripts/lib/fr_registry.rb` 从完整 `HEAD` 祖先历史生成：157 个历史编号 / 162 条历史路径，另有 13 条无 FR 文件历史的审阅例外；5 个编号存在多路径碰撞。浅克隆拒绝生成。
...
| FR-174 | PR 反馈延迟由治理决定 —— 关键路径 22 分钟，其中最长的产品 job 5 分钟 | P2 | In Progress | git history |
