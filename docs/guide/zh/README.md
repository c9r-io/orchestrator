...
本指南帮助你从 **Harness Engineering control plane** 的视角理解和使用 Agent Orchestrator，把基于 shell 的 agent 工作方式制度化为可复用的软件交付流程。

## 适用人群

- **新用户**：希望快速上手
- **工作流作者**：设计可复用的 agent harness 与 SDLC 流水线
- **平台建设者**：希望把 shell-based agent 融入团队工程体系
...
| 章节 | 主题 | 难度 |
|------|------|------|
| [00 - 愿景](00-vision.md) | 理解 Harness Engineering control plane 的定位 | 入门 |
...
- **OpenAI 概念参考**：[Harness Engineering](https://openai.com/index/harness-engineering/) —— 本项目明确采用的工程 framing

## 入口

编排器使用 C/S 架构，守护进程长期运行并承担本地控制面的角色，CLI 作为轻量 gRPC 客户端：
