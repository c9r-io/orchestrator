...
2.  **Orchestrator Engine** (`core/` + `crates/orchestrator-scheduler/`):
    ...
    *   **Coordination Tool Host**: For a tool-capable Claude run, the scheduler starts an ephemeral authenticated HTTP callback on loopback. A run-scoped mode-`0600` MCP configuration gives the provider a transport-only `orch-mcp-tools` stdio shim; the shim forwards JSON-RPC to the daemon, where `allowedTools`, state transitions, test execution, ticket operations, and dynamic-item creation are enforced. Provider tool events and authoritative daemon receipts both enter `events`. See [Coordination MCP Tools](design_doc/orchestrator/130-coordination-collapse-mcp-tools.md).
