...
- FR-114 Gateway/daemon logs and audit may include request/intent/installation IDs or stable digests, owner, generation/version, cursor, state, and bounded error code. They must reject app/config/client/signing secrets, bot token, OAuth code/state, poll/pairing secret, raw Authorization/header/body, workspace name, message URL, and provider response body. Transfer diagnostics identify deferred adoption without printing the encrypted handoff or replacement pairing. See `docs/qa/orchestrator/162-managed-slack-connection-shared-oauth.md` Scenarios 3-5.
