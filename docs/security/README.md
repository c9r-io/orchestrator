...
| `docs/security/authorization/02-file-sharing-ceiling.md` | Canonical host-path ceiling, default deny, traversal/symlink resistance, and global Skill provenance | 5 | Critical |
...
11. Before releasing Slack Reaction Skill Automation, run every FR-107 through FR-112 source overlay plus `docs/qa/orchestrator/161-slack-reaction-skill-automation-release.md`; require signed two-badge routing, concurrent/restart convergence, diagnostic redaction, and forward-only compatible rollback as defined in `crates/orchestrator-persistence/src/migration.rs`.
