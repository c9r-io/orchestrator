...
> 1. Add an `Echo` RPC, `EchoRequest` (with a `message` field), and `EchoResponse` (with a `reply` field) in `crates/proto/orchestrator.proto`.
...
3. Code changes (expected to involve `crates/proto/orchestrator.proto`, `crates/daemon/src/server/system.rs`, `crates/cli/src/commands/`).
4. New unit tests.
...
|------|----------------|
| `crates/proto/orchestrator.proto` | Add `rpc Echo`, `message EchoRequest`, `message EchoResponse` |
...
2. `crates/proto/orchestrator.proto` contains the `Echo` RPC definition.
