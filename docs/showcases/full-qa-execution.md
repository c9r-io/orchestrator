...
或需要 GUI 环境（在纯 CLI 回归中不可执行），已标记为 `self_referential_safe: false`。

其中 **34 个被完全跳过**（无 `self_referential_safe_scenarios`），
**7 个被部分执行**（仅限列出的安全场景）。

#### docs/qa/orchestrator/（36 个）
...
| 无 frontmatter 标记（默认 safe） | 20 | 完全执行 |
...
3. 34 个完全不安全文档全部被正确跳过，7 个部分安全文档仅执行了安全场景。
