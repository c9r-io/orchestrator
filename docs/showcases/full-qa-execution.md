...
> 预计约 132 个 item，其中约 39 个会被 prehook 自动跳过（`self_referential_safe: false`），
> 实际执行约 93 个。
...
| 异常 | 判断方式 | 处理 |
|------|---------|------|
| 不安全文档未被跳过 | `step_skipped` 数量 < 39 | 检查 workspace `self_referential` 设置、QA 文档 frontmatter |
...
- **约 93 个安全 QA 文档** × **每个约 2-5 分钟** = 约 50-130 分钟（4 并行）
- 39 个不安全文档被跳过（< 1 秒）
