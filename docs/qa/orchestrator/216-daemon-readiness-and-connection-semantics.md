...
### S3 附：守卫真的会拦住门禁（before/after 实测）

迁移的价值不在于"调用了 helper"，而在于**就绪不来时门禁会停**。用一个 shim 证明：
它把除 `daemon status --wait-ready` 以外的一切转发给真 CLI，而对就绪调用
**先 sleep 8 秒（让 daemon 有时间绑上套接字）再拒绝**——这正是待测状态：
**套接字答得出来，就绪答不出来**。

```bash
ORCH=<shim> bash scripts/qa/test-attention-inbox.sh
```

| 版本 | 退出码 | 实际发生的事 |
| --- | --- | --- |
| 修复前（`\|\| true` + `if ! task list` 守卫） | **0** | 跑完整个正文并报 `Attention Inbox QA: 10 passed, 0 failed` |
| 修复后（守卫直接测就绪） | **1** | 停在守卫处，**0 个场景执行**，日志指名 shim 的拒绝理由 |

修复前那一行"10 passed, 0 failed"就是本需求存在的全部理由：守卫用 `task list`
复探，而 `task list` 在套接字接受连接的那一刻就成功，于是就绪失败被**改写成了通过**。
第一版迁移把 helper 的判定 `|| true` 丢掉、让旧守卫留在原地，等于换了一层皮
——这一条是闭环自检（Phase 5）发现的，不是门禁发现的。
