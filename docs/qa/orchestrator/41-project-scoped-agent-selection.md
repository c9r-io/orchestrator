...
./scripts/orchestrator.sh apply -f <fixture> --project <project>

# Create task in project (agent selection uses project-scoped agents)
./scripts/orchestrator.sh task create --project <project> ...
```
...
1. Reset and apply into project scope:
   ```bash
   ./scripts/orchestrator.sh qa project reset qa-scope --force
   ./scripts/orchestrator.sh apply -f fixtures/manifests/bundles/multi-echo.yaml --project qa-scope
   ```

2. Inspect global vs project config:
   ```bash
   ./scripts/orchestrator.sh manifest export | python3 -c "
   import sys, yaml
...
1. Reset and apply into project scope:
   ```bash
   ./scripts/orchestrator.sh qa project reset qa-iso --force
   ./scripts/orchestrator.sh apply -f fixtures/manifests/bundles/multi-echo.yaml --project qa-iso
   ```
...
1. Reset and apply (project agents only have `qa` capability):
   ```bash
   ./scripts/orchestrator.sh qa project reset qa-fallback --force
   ./scripts/orchestrator.sh apply -f fixtures/manifests/bundles/multi-echo.yaml --project qa-fallback
   ```
...
1. Set up project with ticket files:
   ```bash
   ./scripts/orchestrator.sh qa project reset qa-tickets --force
   ./scripts/orchestrator.sh apply -f fixtures/manifests/bundles/multi-echo.yaml --project qa-tickets
   ```

2. Create some auto-ticket files manually (simulating previous run):
   ```bash
   TICKET_DIR=$(./scripts/orchestrator.sh manifest export | python3 -c "
   import sys, yaml
   cfg = yaml.safe_load(sys.stdin)
...
1. Ensure project agents exist:
   ```bash
   ./scripts/orchestrator.sh qa project reset qa-global --force
   ./scripts/orchestrator.sh apply -f fixtures/manifests/bundles/multi-echo.yaml --project qa-global
   ```
