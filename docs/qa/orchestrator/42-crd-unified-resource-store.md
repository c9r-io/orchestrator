...
|---|----------|--------|-----------|--------|-------|
| 1 | Builtin CRD Bootstrap on Normalize | PASS | 2026-03-31 | claude | 2 targeted tests pass (normalize_config_populates_builtin_crds, normalize_config_rebuilds_resource_store) |
| 2 | CrdProjectable Round-Trip for All 10 Types | PASS | 2026-03-31 | claude | 13 projection tests pass (all round-trips + malformed spec rejection) |
| 3 | Targeted Writeback — write_back_single and remove_from_legacy | PASS | 2026-03-31 | claude | 39 writeback tests pass (seed, reconcile, remove for all builtin kinds) |
| 4 | apply_to_store / delete_from_store Integration | PASS | 2026-03-31 | claude | 10 tests: apply_to_store (5), delete_from_store (3), metadata_from_store (2). Test filter paths use `resource::tests::cases::` (not `resource::tests::`) |
| 5 | ResourceStore Edge Cases — Key Isolation and Corruption Resilience | PASS | 2026-03-31 | claude | 22 store tests pass (cross-kind isolation, generation, namespaced keys, singleton projection, corruption resilience) |
