...
**Entry points**:
- `./scripts/orchestrator.sh apply -f <manifest.yaml>` — resources flow through ResourceStore
- `./scripts/orchestrator.sh get <kind>/<name> -o yaml` — reads from legacy fields (projection cache)
- `./scripts/orchestrator.sh delete <kind>/<name>` — removes from both store and legacy
- `./scripts/orchestrator.sh manifest export -o yaml` — exports from legacy fields
...
3. Verify the agent exists:
   ```bash
   ./scripts/orchestrator.sh get agent/test-bootstrap-agent -o yaml
   ```
