...
| 2 | Public DB Facade No Longer Exposes `ensure_column` | PASS | 2026-03-28 | Claude | `rg` confirmed no public helper and no `crate::db::ensure_column` call sites remain |
...
| 4 | Local Workflow Store Uses Repository-Backed Persistence | PASS | 2026-03-28 | Claude | All store tests passed; CRUD/upsert/list/prune semantics intact |
| 5 | Full Package Regression Remains Green After Persistence Refactor | PASS | 2026-03-28 | Claude | 1451 lib tests passed; no regressions in scheduler/session/store/migration |
