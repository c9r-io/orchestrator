...
> **Node version.** The GUI suite requires Node within the range
> `gui/package.json` declares (`>=22 <26`); CI pins 22. Under Node 26, twelve
> tests unrelated to this FR fail on `localStorage` being undefined in jsdom, and
> `scripts/coverage-governance.sh` fails with them because it runs that suite.
> Check `node -v` before reading any GUI result as evidence.
...
- `gives every page exactly one url segment` passes, deriving the comparison from
  `pathForPage` so the nav — which builds hrefs directly — and
  `formatConsoleRoute` cannot disagree.
- `keeps hashes minted before the Task rename landing on the same page` passes:
  `#/processes`, `#/processes/task-1`, `#/processes/task-1?review=safe-resume`
  and `#/new-process/draft-1` parse to the same routes as their new spellings,
  **and** `formatConsoleRoute` writes back `#/tasks/task-1` and
  `#/new-task/draft-1`.

**Why the halves are asserted together**: a rename that touched only the label
leaves the nav linking `#/processes`; one that touched only the route leaves the
console saying Processes. Each passes an assertion reading only the other. The
write-back assertion is the same trap in the compatibility direction — an alias
that also became the canonical output would leave the console showing Tasks and
minting `#/processes` forever, which every "the old link still works" test passes.

**Browser-level evidence**: `gui/tests/e2e/process-console.spec.ts` keeps one case
(`non-code process uses task semantics in the console`) deliberately on
`/#/processes/task-non-code`, so the alias is exercised through a real browser and
not only through `parseConsoleRoute`.

## Scenario 2 — a blanket 进程 → 任务 substitution is rejected
...
- `calls a pending task a draft rather than a wish` passes, and no string in
  `wishPool` contains 许愿.
...
daemon-diagnostic string. That is the point — Chinese collapses two unrelated
senses onto 进程, and a substitution pass turns a daemon-down diagnostic into
advice for a different problem while satisfying every vocabulary assertion.

## Scenario 3 — the derived gates and the document gates pass on the real tree
...
All five gates exit `0`, each log ending in its own summary line —
`=== docs reality alignment: 7 passed, 0 failed ===` in particular, because a run
that terminates early reads exactly like a complete one from the exit code alone.
`check_resource_kind_catalog` and `check_guide_language_parity` both pass.

The measurement prints exactly four files above 0.50 — the four Slack runbooks —
and those four are exactly the slugs `translationGaps` declares for the guide
collection. `agent-driver-model.md` scores about 0.37 and is deliberately below
the line: it is an English chapter containing Chinese passages, a different
problem recorded as a known limit in DD-182.

Publishing integrity must pass `check_nav_complete`: the new English chapter 08 is
published, so `site/.vitepress/config.ts` has to link it or the page is generated
and unreachable. `test-cli-doc-parity.sh` is expected to be **unaffected** — it
governs `07-cli-reference.md` in both locales, and chapter 08 is a console guide
rather than a CLI surface. The FR asked for that scope to be evaluated; the answer
was that 08 does not belong there, and `check_guide_language_parity` covers the
EN/ZH chapter-set question instead.

## Scenario 4 — both new gates fail in both directions, naming what broke
...
## Scenario 5 — the guide's Trigger examples are accepted by the product
...
`task_completed`, `webhook` and `filesystem` to be documented, and checks that the
filesystem example's debounce value reaches the field rather than merely parsing.
...
- [ ] S1: nav label and nav href both read Task from the same element; pre-rename
      hashes parse and the canonical output is the new spelling
- [ ] S2: the daemon-sense 进程 string survives a vocabulary sweep
- [ ] S3: the five document gates pass with their summary lines; the Chinese files
      in the EN slot are exactly the declared ones
- [ ] S4: 21 fixtures pass, and the four new ones assert their diagnostics
- [ ] S5: the chapter's Trigger examples deserialize; `debounceMs` fails
