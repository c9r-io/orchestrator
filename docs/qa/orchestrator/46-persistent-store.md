...
CLI surface: `./scripts/orchestrator.sh store get|put|delete|list|prune`.
...
### Preconditions
- Orchestrator binary is built and accessible via `./scripts/orchestrator.sh`
...
1. Put a JSON value:
   ```bash
   ./scripts/orchestrator.sh store put metrics bench_001 '{"test_count": 1334, "pass_rate": 0.98}'
   ```
2. Get the value back:
   ```bash
   ./scripts/orchestrator.sh store get metrics bench_001
   ```
3. Put a second entry:
   ```bash
   ./scripts/orchestrator.sh store put metrics bench_002 '{"test_count": 1400, "pass_rate": 0.99}'
   ```
4. List all entries in the store:
   ```bash
   ./scripts/orchestrator.sh store list metrics
   ```
5. Delete the first entry:
   ```bash
   ./scripts/orchestrator.sh store delete metrics bench_001
   ```
6. Verify deletion:
   ```bash
   ./scripts/orchestrator.sh store get metrics bench_001
...
2. Apply the manifest:
   ```bash
   ./scripts/orchestrator.sh apply -f test-store.yaml
   ```
...
4. Put a valid value:
   ```bash
   ./scripts/orchestrator.sh store put validated-metrics run_001 '{"test_count": 50, "pass_rate": 0.95}'
   ```
5. Put an invalid value (negative test_count violates `minimum: 0`):
   ```bash
   ./scripts/orchestrator.sh store put validated-metrics run_002 '{"test_count": -1, "pass_rate": 0.5}'
   ```
6. Put an invalid type (string instead of object):
   ```bash
   ./scripts/orchestrator.sh store put validated-metrics run_003 '"not an object"'
   ```
...
2. Apply the manifest:
   ```bash
   ./scripts/orchestrator.sh apply -f test-command-provider.yaml
   ```
...
4. Put a value via command adapter:
   ```bash
   ./scripts/orchestrator.sh store put mock-store key1 '{"data": "hello"}'
   ```
5. Get the value back:
   ```bash
   ./scripts/orchestrator.sh store get mock-store key1
   ```
6. Delete the value:
   ```bash
   ./scripts/orchestrator.sh store delete mock-store key1
   ```
7. Verify deletion:
   ```bash
   ./scripts/orchestrator.sh store get mock-store key1
...
1. Put entries in two different projects:
   ```bash
   ./scripts/orchestrator.sh store put shared k1 '{"v": 1}' --project proj-a
   ./scripts/orchestrator.sh store put shared k2 '{"v": 2}' --project proj-a
   ./scripts/orchestrator.sh store put shared k1 '{"v": 99}' --project proj-b
   ```
2. List entries for `proj-a`:
   ```bash
   ./scripts/orchestrator.sh store list shared --project proj-a
   ```
3. List entries for `proj-b` in JSON format:
   ```bash
   ./scripts/orchestrator.sh store list shared --project proj-b -o json
   ```
4. Upsert: overwrite `k1` in `proj-a`:
   ```bash
   ./scripts/orchestrator.sh store put shared k1 '{"v": 100}' --project proj-a
   ```
5. Verify upsert:
   ```bash
   ./scripts/orchestrator.sh store get shared k1 --project proj-a
   ```
6. Verify `proj-b` is unaffected:
   ```bash
   ./scripts/orchestrator.sh store get shared k1 --project proj-b
   ```
