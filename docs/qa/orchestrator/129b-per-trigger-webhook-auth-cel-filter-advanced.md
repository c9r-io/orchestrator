...
- [ ] **S6: Global secret fallback** — CANNOT VERIFY (daemon not running with `--webhook-secret global-key`; global secret derived from control-plane CA cert instead). A ticket was auto-filed for this scenario and has since been resolved and removed.
