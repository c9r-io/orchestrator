...
2. Verify the new flags appear in help:
   ```bash
   ./scripts/orchestrator.sh --help | rg -- "--log-level|--log-format"
   ```
...
1. Capture stdout and stderr separately:
   ```bash
   ./scripts/orchestrator.sh init > /tmp/orch-init-stdout.txt 2> /tmp/orch-init-stderr.txt
   ```
...
1. Run `init` with JSON console logging:
   ```bash
   ./scripts/orchestrator.sh --log-format json init > /tmp/orch-json-stdout.txt 2> /tmp/orch-json-stderr.txt
   ```
2. Inspect stderr:
   ```bash
