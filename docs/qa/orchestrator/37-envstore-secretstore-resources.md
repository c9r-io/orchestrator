...
1. Create a multi-document YAML file `test-env-stores.yaml`:
   ...
   ```
2. Run `./scripts/orchestrator.sh apply -f test-env-stores.yaml`
3. Run `./scripts/orchestrator.sh manifest export`
...
### Steps
1. Run `./scripts/orchestrator.sh apply -f test-env-stores.yaml` again (same file from Scenario 1)
...
### Steps
1. Run `./scripts/orchestrator.sh delete env-store shared-config`
2. Run `./scripts/orchestrator.sh manifest export` — verify `shared-config` is absent
3. Re-apply `shared-config` from Scenario 1 YAML
4. Run `./scripts/orchestrator.sh delete secret-store api-keys`
5. Run `./scripts/orchestrator.sh manifest export` — verify `api-keys` is absent, `shared-config` still present
...
1. Create `bad-env-store.yaml`:
   ...
   spec:
     data:
       KEY: "value"
   ```
2. Run `./scripts/orchestrator.sh apply -f bad-env-store.yaml`
