...
|---|----------|--------|-----------|--------|-------|
| 1 | Invariant passes — command exit code check | ☐ | | | |
| 2 | Invariant fails — command returns wrong exit code | ☐ | | | |
| 3 | Protected file modification detection | ☐ | | | |
| 4 | Invariant with warn-only violation | ☐ | | | |
| 5 | Checkpoint filtering — invariants run only at configured points | ☐ | | | |
| G | Invariant immutability (pinned at task start) | ☐ | | | |
