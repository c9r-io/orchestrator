...
| orchestrator | `docs/qa/orchestrator/207-pipeline-variable-surface-retirement.md` | 6 | FR-156: the step-level pipeline-variable authoring surface (`store_inputs`, `store_outputs`, `step_vars`, `store_put` post-action) rejected at apply with a field-naming diagnostic and recursing into `chain_steps`; per-object migration parity against a baseline recorded before the removal, which found a `promotion` branch that had never once run and a `store put`/`get` pair that were not inverses; `PipelineVariables` recorded as a retained carrier rather than debt, with `--emit-consumers` regenerating the count |
