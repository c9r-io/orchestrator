...
| orchestrator | `docs/qa/orchestrator/185-persistence-dependency-chokepoint.md` | 5 | FR-136 the chokepoint decision scoped to the orchestrator database, every driver reference outside core classified with the coverage derived rather than claimed, a rule proven to discriminate by a crate it must NOT reject, SQL caught in a file naming no driver at all, a new member discovered outside `crates/` by manifest and source both, and no production code introduced |
